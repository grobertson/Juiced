...
def test_list_and_load_themes(tmp_path):
    ...
    t1.write_text(json.dumps({'name': 'Blue', 'colors': {'status_bar': {'background': 'blue', 'text': 'white'}}}))
    t2 = themes_dir / 'simple.json'
    t2.write_text(json.dumps({'name': 'Simple', 'colors': {'status_bar': {'background': 'cyan', 'text': 'black'}}}))
...
def test_change_theme_creates_config_when_missing(tmp_path):
    ...
    theme_file.write_text(json.dumps({'name': 'MyTheme', 'colors': {'status_bar': {'background': 'green', 'text': 'black'}}}))
