...
def test_tab_completion_username_cycle():
    ...
    users = {'alice': SimpleNamespace(name='alice'), 'alex': SimpleNamespace(name='alex'), 'bob': SimpleNamespace(name='bob')}
...
async def test_process_command_sends_chat_and_handles_slash(monkeypatch):
    ...
    monkeypatch.setattr(type(bot), 'chat', lambda self, msg: asyncio.ensure_future(fake_chat(msg)))
    monkeypatch.setattr(type(bot), 'handle_slash_command', lambda self, text: asyncio.ensure_future(fake_handle_slash(text)))
