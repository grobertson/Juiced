...
def make_item(uid, title="T", duration=10):
    return SimpleNamespace(
        ...
        duration=duration,
        seconds=0,
        link=SimpleNamespace(id=str(uid), type="yt"),
    )
...
async def test_info_status_and_users_and_user_and_afk():
    ...
    users = [
        SimpleNamespace(name="alice", rank=2, afk=False),
        SimpleNamespace(name="bob", rank=1, afk=False),
    ]
    ...
    bot.channel = SimpleNamespace(
        name="chan",
        userlist=userlist,
        playlist=SimplePlaylist(queue=[make_item(1)], current=None),
    )

    # cmd_info
    await bot.cmd_info()
    assert any(
        "Bot" in m["message"] or "Channel" in m["message"] for m in bot.chat_history
    )

    # cmd_status
    await bot.cmd_status()
    assert any(
        "Connected" in m["message"] or "Uptime" in m["message"]
        for m in bot.chat_history
    )
...
async def test_playlist_commands_add_remove_move_jump_and_pause_and_kick(monkeypatch):
    ...
    bot.channel = SimpleNamespace(
        name="chan", userlist=SimpleNamespace(count=2, leader=bot.user), playlist=pl
    )
    ...
    async def fake_add_media(link, append=True, temp=True):
        return {
            "item": {
                "media": {
                    "type": link.type if hasattr(link, "type") else "yt",
                    "id": getattr(link, "id", "x"),
                }
            }
        }
    ...
    monkeypatch.setattr(
        type(bot),
        "add_media",
        lambda self, link, append=True, temp=True: asyncio.sleep(0),
    )
    ...
    assert any(
        "Added" in m["message"] or "Failed" in m["message"] for m in bot.chat_history
    )
    ...
    assert any(
        "Removed" in m["message"]
        or "No playlist" in m["message"]
        or "Position" in m["message"]
        for m in bot.chat_history
    )

    # move - provide enough args
    monkeypatch.setattr(
        type(bot), "move_media", lambda self, item, after: asyncio.sleep(0)
    )
    await bot.cmd_move("1 2")
    assert any(
        "Moved" in m["message"] or "Usage" in m["message"] for m in bot.chat_history
    )

    # jump
    monkeypatch.setattr(
        type(bot), "set_current_media", lambda self, item: asyncio.sleep(0)
    )
    await bot.cmd_jump("1")
    assert any(
        "Jumped" in m["message"] or "No playlist" in m["message"]
        for m in bot.chat_history
    )
    ...
    # kick
    monkeypatch.setattr(
        type(bot), "kick", lambda self, username, reason="": asyncio.sleep(0)
    )
    await bot.cmd_kick("alice")
    assert any(
        "Jumped" in m["message"] or "AFK" in m["message"] or "Removed" in m["message"]
        for m in bot.chat_history
    )
