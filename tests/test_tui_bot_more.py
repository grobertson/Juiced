...
def test_render_top_and_bottom_status_outputs():
    ...
    bot.channel = SimpleNamespace(
        name="mychan", playlist=FakePlaylist(), userlist=FakeUserList([], count=5)
    )
...
def test_render_users_outputs():
    bot = make_bot()
    users = [
        FakeUser("owner", rank=4),
        FakeUser("mod", rank=2),
        FakeUser("alex", rank=1, afk=True),
        FakeUser("bob", rank=0),
    ]
...
def test_handle_media_change_with_uid_and_playlist():
    ...
    asyncio.get_event_loop().run_until_complete(
        bot.handle_media_change("setCurrent", 42)
    )
