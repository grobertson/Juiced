...
# Fallback for static analysis; conftest will override at runtime
_TEST_LOG_DIR = None
...
async def test_current_no_media_and_with_current():
    ...
    assert any(
        "No media currently playing" in m["message"] or "No media" in m["message"]
    ...
    await bot.handle_slash_command("/current")
    assert any(
        "Current Media Info" in m["message"] or "Title:" in m["message"]
        for m in bot.chat_history
    )
...
async def test_move_to_beginning_not_supported_and_move_usage():
    ...
    assert any(
        "Moving to beginning not yet supported" in m["message"]
        for m in bot.chat_history
    )
