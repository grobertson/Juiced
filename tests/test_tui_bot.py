...
def make_bot():
    ...
    bot = tui_mod.TUIBot(tui_config={}, config_file=str('config.yaml'), domain='example.com', channel='test')
    return bot
...
def test_calculate_message_wrapped_lines():
    ...
    wrapped = bot._calculate_message_wrapped_lines('This is a long message', chat_width=20, timestamp='12:00:00', username='u', prefix='')
...
async def test_process_command_toggle_and_pm(monkeypatch):
    ...
    assert any(msg.get('prefix', '').startswith('[PM') or msg.get('prefix') == '[PM->]' for msg in bot.chat_history)
